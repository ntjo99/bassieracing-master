...
class Game:
    ...
    # The game loop
    def start(self, genome, net):
        lastTime = time.time()
        self.time = lastTime
        self.genome = genome
        self.net = net
        map_path = r"c:/Users/Nathan/Desktop/Algo/bassieracing-master/assets/maps/classic-eight.json"
        self.map = Map.load_from_file(map_path)
        self.gamemode = GameMode.SINGLE_PLAYER
        self.page = SelectVehiclePage(self, GameMode.SINGLE_PLAYER, map)
        self.page = GamePage(self, GameMode.SINGLE_PLAYER, self.map, [
                {
                    'type': vehicles[0],
                    'color': 0#self.game.settings['selected']['left']['vehicle-color']
                }
            ])
        while self.running:
...
def evalT(winner, config):
    
    winner.fitness = 0
        
    net = neat.nn.FeedForwardNetwork.create(winner, config)
    game = Game()
    game.start(winner, net)
    
def eval(genomes, config):
    config.fitness_threshold = 150
    for genome_id, genome in genomes:
        genome.fitness = 0
        
        net = neat.nn.FeedForwardNetwork.create(genome, config)
        game = Game()
        game.start(genome, net)
neatCall()
# genome = 0
# net = 0
# game = Game()
