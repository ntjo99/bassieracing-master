...
class Vehicle:
    ...
    def check_crash(self):
        ...
        if self.game.time - self.crashTime > 0.3: #self.map.crashes['timeout']:
            # Crash the vehicle
            self.crashed = True
            self.crashTime = self.game.time
    ...
    def update(self, delta, camera):
        ...
        if not self.started or self.finished:
            return
        input_vals = [self.f, self.fe, self.fw, self.fee, self.fww]
        output_vals = self.game.net.activate(input_vals)
        outputIndex = output_vals.index(max(output_vals[-3:]))
        # if(output_vals[0] > .1):
        self.moving = Vehicle.MOVING_FORWARD
        # else:
        #     self.moving = Vehicle.NOT_MOVING
        if (outputIndex == 1):
            self.turning = Vehicle.NOT_TURNING
        elif (outputIndex == 2):
            self.turning = Vehicle.TURNING_LEFT
        else:
            self.turning = Vehicle.TURNING_RIGHT
        # When crashed
        if self.crashed:
